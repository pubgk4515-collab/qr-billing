...
export default function AdminDashboard({ params }: { params: Promise<{ store_slug: string }> }) {
  ...
  const theme = {
    ...
    surface: isDark ? 'bg-[#0A0A0A]' : 'bg-[#FAFAFA]',
    ...
    textFaint: isDark ? 'text-zinc-600' : 'text-zinc-400',
    divider: isDark ? 'bg-white/5' : 'bg-black/5',
    input: isDark ? 'bg-[#111] border-white/10' : 'bg-white border-black/10',
    primaryBtn: isDark
    ...
    accentBg: (alpha: number) => (isDark ? `${themeColor}${alpha}` : `${themeColor}${Math.min(alpha * 2, 100)}`),
  };
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} font-sans pb-16 relative ${inter.variable}`}
    ...
    >
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="font-semibold text-sm tracking-tight leading-none">{storeData?.store_name}</h1>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 flex flex-col gap-16">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            ...
            <form onSubmit={handleManualApproveSubmit} className="flex w-full sm:w-auto">
              <input
                ...
                className={`w-full sm:w-40 py-2 pl-4 pr-2 text-sm font-medium rounded-l-xl border ${theme.input} focus:outline-none focus:ring-1 focus:ring-white/20 placeholder:font-normal`}
              />
              <button
                type="submit"
                disabled={!manualApproveCartId}
                className="px-4 text-sm font-semibold text-white rounded-r-xl bg-white/10 hover:bg-white/20 disabled:opacity-50 transition-all border border-white/10 border-l-0"
              >
                Approve
              </button>
            </form>
          ...
          <div className="grid grid-cols-1 gap-4">
            <AnimatePresence>
              {pendingOrders.length === 0 ? (
                <div className={`${theme.cardAlt} border ${theme.border} rounded-[2rem] p-10 flex flex-col items-center justify-center text-center opacity-60`}>
                  ...
                  >
                    ...
                    <div className="pl-4">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-sm text-zinc-400 font-medium">{order.customer_phone}</span>
                      </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            ...
            <form onSubmit={handleManualBillSubmit} className="flex w-full sm:w-auto">
              <input
                ...
                className={`w-24 py-2 px-3 text-sm font-medium rounded-l-xl border border-r-0 ${theme.input} placeholder:font-normal`}
              />
              <input
                type="tel"
                maxLength={10}
                ...
                className={`w-28 py-2 px-3 text-sm font-medium border border-x-0 ${theme.input} placeholder:font-normal`}
              />
              <button
                type="submit"
                disabled={!manualBillCartId || manualBillPhone.length < 10}
                className="px-4 text-sm font-semibold text-[#25D366] bg-[#25D366]/10 hover:bg-[#25D366]/20 disabled:opacity-50 rounded-r-xl transition-all border border-[#25D366]/20 border-l-0 flex items-center justify-center"
              >
          ...
          <div className="grid grid-cols-1 gap-3">
            <AnimatePresence>
              {billRequests.length === 0 ? (
                <div className={`${theme.cardAlt} border ${theme.border} rounded-2xl p-6 text-center opacity-60`}>
                ...
                billRequests.map((request) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 font-medium">{request.customer_phone}</p>
                    </div>
                    <button
        ...
        {/* 3. CHECKOUT CONSOLE */}
        <section className={`${theme.cardAlt} border ${theme.borderLight} rounded-[2.5rem] overflow-hidden flex flex-col h-[550px] relative`}>
          <AnimatePresence>
            {itemFetching && (
              <motion.div
                initial={{ opacity: 0 }}
          ...
          <div className={`p-5 border-b ${theme.border} flex flex-col sm:flex-row sm:items-center justify-between gap-4`}>
            <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
              ...
              <form onSubmit={handleManualTagSubmit} className="flex w-full sm:w-auto">
                <input
                  ...
                  placeholder="Tag ID…"
                  className={`w-full sm:w-36 py-2.5 pl-4 pr-2 text-sm font-medium uppercase rounded-l-xl border border-r-0 ${theme.input} focus:outline-none placeholder:normal-case placeholder:font-normal`}
                />
                <button
                  type="submit"
          ...
          <div className={`flex-1 overflow-y-auto p-4 flex flex-col gap-2 ${isDark ? 'bg-[#050505]/50' : 'bg-gray-50'}`}>
            <AnimatePresence>
              {scannedItems.length === 0 ? (
                ...
                scannedItems.map((item) => (
                  ...
                  >
                    <div>
                      <p className="text-[10px] font-semibold text-zinc-500 uppercase">{item.tag}</p>
          ...
          <div className={`p-5 border-t ${theme.border} flex flex-col gap-4`}>
            <div className="relative">
              ...
              <input
                ...
                className={`w-full py-3 pl-12 pr-4 text-sm font-medium rounded-xl border ${theme.input} focus:outline-none`}
              />
            </div>
            <button
      ...
      <AnimatePresence>
        {isViewModalOpen && viewingOrder && (
          ...
          >
            ...
            >
              <button
                onClick={() => { setIsViewModalOpen(false); setViewingOrder(null); }}
              ...
              <div className={`mb-6 pb-6 border-b ${theme.border}`}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400">Order Details</span>
                </div>
                <h2 className="text-2xl font-semibold tracking-tight mt-2 mb-1">{viewingOrder.cart_id}</h2>
                <div className="flex items-center gap-3">
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                    viewingOrder.payment_method === 'ONLINE' ? 'bg-blue-500/10 text-blue-400' : 'bg-amber-500/10 text-amber-400'
                  }`}>
              ...
              <div className="flex-1 overflow-y-auto pr-2 mb-6 flex flex-col gap-3">
                {viewingOrder.purchased_items && viewingOrder.purchased_items.map((item: any, idx: number) => (
                  <div key={idx} className={`${theme.card} border ${theme.border} rounded-2xl p-3 flex items-center gap-4`}>
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-sm leading-tight mb-1">{item.products?.name || 'Item'}</h4>
                      <span className="text-[9px] font-mono font-semibold uppercase tracking-widest text-zinc-500 bg-white/5 px-2 py-0.5 rounded">{item.id}</span>
                    </div>
