      ...
      >
        <header className={`sticky top-0 z-30 px-6 py-3 border-b ${theme.nav}`}>
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => router.push(`/admin/${safeStoreSlug}`)} className={`p-2 rounded-full transition-colors ${theme.secondaryBtn}`}>
                <ArrowLeft className="w-4 h-4" />
              </button>
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${themeColor}20`, color: themeColor }}>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setIsDark(!isDark)} className={`p-2 rounded-full transition-all duration-200 ${theme.secondaryBtn} ${pressable}`} aria-label="Toggle theme">
              ...
              <button onClick={() => router.push(`/admin/${safeStoreSlug}/inventory/worker-mode`)} className={`px-4 py-2 rounded-full text-xs font-semibold flex items-center gap-2 transition-all ${theme.primaryBtn}`}>
        ...
        <main className="max-w-5xl mx-auto px-6 py-8 flex flex-col gap-10 mt-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <button onClick={() => setIsGenerateModalOpen(true)} className={`${theme.card} border ${theme.border} py-5 px-2 rounded-[1.5rem] flex flex-col items-center justify-center gap-2 transition-all ${pressable} hover:border-white/20`}>
            ...
            <button onClick={() => { setBindingTagId(null); setIsAddModalOpen(true); }} className={`py-5 px-2 rounded-[1.5rem] flex flex-col items-center justify-center gap-2 transition-all ${pressable} ${theme.primaryBtn}`}>
            ...
            <button onClick={() => setIsPrintModalOpen(true)} className={`${theme.card} border ${theme.border} py-5 px-2 rounded-[1.5rem] flex flex-col items-center justify-center gap-2 transition-all ${pressable} hover:border-white/20`}>
          ...
          <div className="flex flex-col gap-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Search by Tag or Name…" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full rounded-2xl py-3.5 pl-12 pr-4 text-sm font-medium border ${theme.input} ${theme.placeholder} focus:outline-none`} />
            </div>
            <div className={`flex gap-2 p-1.5 rounded-2xl border ${theme.border} overflow-x-auto`}>
              {['all', 'active', 'in_cart', 'sold', 'free'].map((f) => (
                <button key={f} onClick={() => setActiveFilter(f)} className={`flex-1 min-w-[65px] py-2.5 rounded-xl text-[10px] font-semibold uppercase tracking-widest transition-all ${activeFilter === f ? theme.primaryBtn + ' shadow-sm' : `${theme.textMuted} hover:${theme.text}`}`}>
                  {f.replace('_', ' ')}
                </button>
              ))}
            </div>
          ...
          {/* 🟢 THE PHYSICAL PRODUCT CARD LAYER */}
            ...
            <AnimatePresence mode="popLayout">
              {filteredInventory.map((item) => (
                <motion.div
                  ...
                  className={`group relative p-5 overflow-hidden transition-transform duration-500 ease-out hover:-translate-y-[2px] ${
                    (item.status === 'free' || item.status === 'sold')
                      ? `rounded-[2rem] border-dashed ${theme.borderDashed} bg-transparent`
                      : `rounded-l-[28px] rounded-r-[48px] border ${isDark ? 'border-white/[0.08]' : 'border-black/[0.05]'}`
                  }`}
                  style={{
                    background: (item.status === 'active' || item.status === 'in_cart')
                      ? (isDark ? 'linear-gradient(135deg, #161618 0%, #0a0a0c 100%)' : 'linear-gradient(135deg, #ffffff 0%, #f4f4f5 100%)')
                      : undefined,
                    boxShadow: (item.status === 'active' || item.status === 'in_cart')
                      ? (isDark ? 'inset 0 0 0.5px rgba(255,255,255,0.1), inset 0 0 40px rgba(0,0,0,0.4)' : 'inset 0 0 0.5px rgba(0,0,0,0.05), inset 0 0 40px rgba(0,0,0,0.02)')
                      : undefined
                  }}
                >
                  {/* ✨ 3D RADIAL LIGHT SOURCE */}
                    ...
                    <div
                      ...
                      style={{
                        background: isDark
                          ...
                          : 'radial-gradient(circle at 80% 20%, rgba(0,0,0,0.02), transparent 60%)'
                      }}
                    />
                  )}

                  {/* 🖼️ THE EMBEDDED PHYSICAL IMAGE */}
                  {(item.status === 'active' || item.status === 'in_cart') ? (
                    item.products?.image_url ? (
                      <div className="absolute right-0 top-0 bottom-0 w-[65%] pointer-events-none z-0 overflow-hidden rounded-r-[48px]">
                        
                        <img
                          src={item.products.image_url}
                          alt=""
                          className="w-full h-full object-cover mix-blend-soft-light opacity-[0.32] group-hover:opacity-[0.38] transform scale-[1.08] translate-x-[6px] group-hover:scale-[1.12] transition-all duration-[500ms] ease-out"
                          style={{
                        ...
                        {/* Soft Status Overlay Tints */}
                        {item.status === 'active' && <div className={`absolute inset-0 mix-blend-overlay ${isDark ? 'bg-emerald-500/5' : 'bg-emerald-500/10'}`} />}
                        {item.status === 'in_cart' && <div className={`absolute inset-0 mix-blend-overlay ${isDark ? 'bg-amber-500/[0.04]' : 'bg-amber-500/[0.07]'}`} />}
                      </div>
                    ) : (
                  ...
                  {/* 🎮 CONTENT LAYER (Preserved logic, high z-index) */}
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        <span
                          className={`text-[11px] font-semibold tracking-widest uppercase ${
                            item.status === 'active' ? 'text-emerald-400' : item.status === 'in_cart' ? 'text-amber-400' : theme.textFaint
                          }`}
                        ...
                        <h3
                          ...
                          style={{ textShadow: (isDark && (item.status === 'active' || item.status === 'in_cart')) ? '0 1px 1px rgba(0,0,0,0.6)' : undefined }}
                        >
                      ...
                      <div className="flex gap-1.5 relative z-20">
                        <button onClick={() => openQrModal(item)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${pressable} backdrop-blur-md ${isDark ? 'bg-white/5 hover:bg-white/10 border-white/10 text-white' : 'bg-black/5 hover:bg-black/10 border-black/5 text-black'} border`}>
                          <QrCode className="w-4 h-4" />
                        </button>
                        {(item.status === 'active' || item.status === 'in_cart') && (
                          <>
                            <button onClick={() => openEditModal(item)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${pressable} backdrop-blur-md ${isDark ? 'bg-white/5 hover:bg-white/10 border-white/10 text-white' : 'bg-black/5 hover:bg-black/10 border-black/5 text-black'} border`}>
                              <Edit2 className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => handleUnbindItem(item.id)} className={`w-10 h-10 rounded-full flex items-center justify-center hover:bg-rose-500/20 transition-all backdrop-blur-md border ${isDark ? 'bg-rose-500/10 border-rose-500/10 text-rose-400' : 'bg-rose-500/5 border-rose-500/20 text-rose-600'}`}>
                      ...
                      {(item.status === 'active' || item.status === 'in_cart') ? (
                        <>
                          <div
                            ...
                            style={{ textShadow: isDark ? '0 1px 1px rgba(0,0,0,0.6)' : undefined }}
                          >
                            ₹{item.products?.price}
                          </div>
                          
                          {item.status === 'in_cart' ? (
                            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full backdrop-blur-md border ${isDark ? 'bg-amber-500/10 border-amber-500/10' : 'bg-amber-500/5 border-amber-500/20'}`}>
                              <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse shadow-[0_0_5px_rgba(251,191,36,0.5)]" />
                              <span className={`text-[9px] font-semibold uppercase tracking-widest ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>In Bag</span>
                            </div>
                          ) : (
                            <div
                              className={`text-[9px] font-semibold px-3 py-1.5 rounded-full uppercase tracking-widest backdrop-blur-md border ${isDark ? 'border-white/5' : 'border-black/5'}`}
                              style={{ backgroundColor: isDark ? `${themeColor}10` : `${themeColor}15`, color: themeColor }}
                            >
                              Active
                            </div>
                          )}
                        ...
                        <>
                          ...
                          <button onClick={() => openAddModalForSpecificTag(item.id)} className={`px-5 py-2.5 rounded-xl text-xs font-semibold transition-all ${pressable} ${theme.primaryBtn}`}>
                            Link Item
                          </button>
                        </>
                      )}
        ...
        <div className="fixed bottom-6 right-6 sm:bottom-10 sm:right-10 z-40 flex flex-col items-end gap-3">
          <button onClick={() => router.push(`/admin/${safeStoreSlug}/analytics`)} className="px-4 py-3 rounded-full flex items-center gap-2 transition-all hover:scale-105 active:scale-95 shadow-lg" style={{ backgroundColor: themeColor, color: '#000' }}>
            <BarChart3 className="w-4 h-4" />
            <span className="text-xs font-semibold hidden sm:block">Analytics</span>
          </button>
          <button onClick={() => router.push(`/admin/${safeStoreSlug}`)} className={`px-4 py-3 rounded-full flex items-center gap-2 transition-all hover:scale-105 active:scale-95 shadow-lg border ${theme.card} ${theme.borderLight}`}>
